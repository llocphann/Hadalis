...
> **Current source pass (2026-09-18; local runtime validation still pending):** the newest maintainer visual report is implemented source-side before any further cleanup. Screen Edge horizontal edges now own their `RoundCorner` endpoint decorators in the same layer surface as the solid band/shadow, matching Hug Bar composition; the four independent corner `PanelWindow` surfaces are removed, and straight shadows still stop before each free rounded-corner footprint; connected popup hover owns the full body and debounces the compositor's Bar→popup leave/enter hand-off; corner-clamped connected popups add a tangent reveal component and therefore move diagonally; Sidebar endpoint shoulders follow the mapped body without a second translation; OSK stays resident long enough to slide through its attached top/bottom edge; both Settings overlay styles are larger bottom-connected popup surfaces and demote below active Polkit; compact Right Sidebar Media now places the shared Equalizer immediately below its media player; Dashboard uses the same solid Material layer as connected popups; retired Sidebar Island/Card controls and their search entry are removed; and the narrowly scoped ThinkFan/TLP polkit helpers are promptless only for the active local session. These are source contracts only until the maintainer completes the Niri/Quickshell live pass.
...
- Screen Edge lower corners: maintainer re-tested after reinstall/update and the prior shared-`RoundCorner` change still left the lower white wedges. The remaining mismatch was architectural: bottom corners were separate `PanelWindow` layer surfaces while the visibly correct top Hug corners live inside the Bar surface. Screen Edge now follows that same composition—top/bottom `EdgeWindow` owns both endpoint `RoundCorner` decorators, anchored directly to `edgeBand`; those decorators paint after the straight shadow, while left/right edge shadows stop before their footprint. The four independent corner layer surfaces are removed. Live light-mode four-corner comparison remains.
