...
- **Connected Popup/Sidebar/Dashboard corner work is temporarily deferred.** The attempted exact Screen Edge `PathArc` reuse was reverted cleanly because the runtime Popup/Dashboard silhouette did not yet match the reference. `ConnectedSurfaceJoinFlares.qml` is back on its prior 20px smooth-union approximation and is intentionally decoupled from `appearance.screenEdge.radius` until that work is resumed explicitly.
...
- [ ] **Screen Edge corner radius is configurable in Settings and also defines normal ii Bar endpoint corners.** Default is 25px; Screen Edge and Bar/VerticalBar must use the same canonical inverted-frame radius. Connected Popup/Dashboard/Sidebar shoulders are explicitly excluded for now.
...
- Bar geometry follow-up: the Popup/Dashboard/Sidebar exact-contact-corner experiment was reverted to the prior shared connected-surface baseline. The physical Screen Edge remains the single inverted-frame owner. For a normal non-auto-hide ii Bar, the frame now uses Bar height/VerticalBar width as the owned side inset, exactly like Caelestia treats Bar as a thicker border side; the same `appearance.screenEdge.radius` therefore shapes Bar endpoint corners. Live top/bottom/left/right validation remains.
...
- Screen Edge corner reconstruction: the rejected `4 EdgeWindow + 4 CornerWindow` composition remains retired. The visible Screen Edge is one full-screen odd-even frame geometry: padded outer rectangle minus one rounded inner workspace rectangle. The geometry primitive remains locked and its radius is configurable (25px default, 0–96px). A normal ii Bar changes only the inset of its owned side to Bar thickness, so its two endpoint corners remain part of this same geometry. Connected Popup/Dashboard/Sidebar shoulders are not coupled to this radius. The physical shadow remains an effect on the same Shape. Live validation at native and fractional scale is required.
...
2. **Connected-surface contact-corner reconstruction is deferred.** Placement, slide-under, hover and shadow ownership remain active, but the attempted exact Screen Edge `PathArc` clone for Popup/Dashboard/Sidebar contacts was reverted after runtime did not show the intended silhouette. Keep the restored `ConnectedSurfaceJoinFlares` baseline untouched until this work is resumed explicitly. Current geometry work is limited to Screen Edge + Bar/VerticalBar.
