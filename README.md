...
> **Current source pass (2026-09-18; local runtime validation still pending):** the newest maintainer visual report is implemented source-side before any further cleanup. Screen Edge inner corners now mirror one canonical silhouette so bottom/top geometry cannot diverge; corner-clamped connected popups add a tangent reveal component and therefore move diagonally; Sidebar endpoint shoulders follow the default translated slide instead of disappearing during it; OSK stays resident long enough to slide through its attached top/bottom edge; both Settings overlay styles are bottom-connected popup surfaces and demote below active Polkit; the Bar media popup no longer appends an inactive-player notice below Equalizer; Weather hourly cards use equal arc-length placement on the painted ellipse; and the ThinkFan row uses the compact RPM label/value form. These are source contracts only until the maintainer completes the Niri/Quickshell live pass.
...
- [x] **Left/right Sidebar direct Screen Edge attachment:** the owning SidebarHost is still a physical left/right layer surface with no connector stem, and the visible body underlaps the full Screen Edge band all the way to the physical display edge. The host now reserves vertical decoration room equal to the shared flare radius and renders the same `ConnectedSurfaceJoinFlares` at the upper/lower endpoints of the left/right attachment, using the active sidebar surface color; this keeps the flare outside content reveal clipping. Left/default-right/compact-right surfaces expose their real surface color and keep the attached-side border/shadow suppressed. Live left/right/custom-edge-width validation is pending.
- [x] **Overview Dashboard as connected popup:** Overview keeps connector/stem geometry retired and positions Dashboard at the exact bottom Bar/Screen Edge boundary. Dashboard now owns a dedicated full-body reveal progress: a translated surface layer travels downward by one complete body height behind its clipped resting boundary, while Dashboard presentation mode disables the generic Overview scale/translate/fade so the panel reads as a popup rather than a zooming overlay. `ConnectedSurfaceJoinFlares` supplies both bottom attachment shoulders, joined bottom corners remain square, and `StyledRectangularShadow` uses the configured Screen Edge shadow size/opacity on free sides while suppressing bottom shadow. Reverse close stays mapped through the reveal tail. Live Meta/Super+Space/open-close/search-transition validation is pending.
