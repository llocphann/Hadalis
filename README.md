...
- Screen Edge lower corners: maintainer re-tested multiple source attempts and clarified that the required reference is the complete top Bar/Screen Edge junction, including its shadow, not merely a matching arc primitive. Audit found two concrete coordinate/paint-order mismatches: horizontal Screen Edge inverse corners were anchored at the physical rim instead of the inner boundary of the side Screen Edge, and horizontal shadows were cut away by the corner radius. The source candidate now offsets each horizontal Screen Edge inverse corner by `screenEdge.width`, starts its horizontal shadow at that same inner boundary, and lets that shadow continue underneath the later `RoundCorner` paint exactly like the reference Hug Bar composition. Independent corner `PanelWindow` surfaces remain retired. Live light-mode four-corner comparison is still required before closure.
