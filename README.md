...
- **Heavy panel residency:** Waffle Action Center now releases its hidden `PanelWindow`/media/stack subtree immediately after the shared Waffle exit duration plus a small safety grace instead of keeping it resident for five minutes. Waffle Start Menu and the grid Wallpaper Selector already self-unload their heavy inner windows, so they are lower-priority outer-retention targets. Continue the audit with Wallpaper Launcher/Coverflow and the Waffle shared Overview host.
