...
- **Dashboard/Overview:** the mapped warm lifecycle remains the baseline, but the standalone Dashboard slide is now tuned specifically for the heavy retained widget surface: 32px travel, 360ms `OutCubic` entrance, 260ms `InCubic` exit, plus a temporary smooth scene-graph layer during motion so widget shadows/effects translate as one cached texture. Opacity and scale remain fixed at 1. Live-validate that open/close feels continuous without first-frame hitch or midpoint surge, plus SongRec geometry cleanup, Dashboard <-> Applications fade-through, compact System Monitor readability, horizontal Available Modules overflow and Niri Overview drag/drop/focus stability.
...
7. **Dashboard/Overview:** live-validate the mapped warm Dashboard lifecycle and tuned motion: no widget-tree rebuild after first use; native surface mapped with empty input/render sleep while closed; entrance uses the 32px/360ms `OutCubic` cached slide and exit uses 260ms `InCubic`, with no opacity/scale fade or visible cache hitch. Also visually accept SongRec geometry cleanup, Dashboard <-> Search Applications crossfade, redesigned System Monitor, horizontal Available Modules overflow, and Niri Overview drag/drop/focus stability.
...
7. Live-validate Dashboard mapped warm reopen + smooth motion: sau lần mở đầu tiên không rebuild widget tree; surface vẫn mapped nhưng input rỗng/render sleep khi đóng; mở dùng slide cache 32px/360ms `OutCubic`, đóng 260ms `InCubic`, không opacity/scale fade và không hitch lúc bắt đầu; đồng thời accept SongRec geometry, Dashboard <-> Search Applications crossfade, System Monitor layout, Available Modules horizontal overflow và Overview drag/drop.
