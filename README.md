...
- [x] **Media Popup equalizer:** the existing CAVA -> `PlayerControl` -> `WaveVisualizer` path remains the single visualizer path. `WaveVisualizer` now renders a Serpantinum-inspired rounded equalizer bar field directly in QML (center-mirrored spectrum, edge fade and animated bar response) instead of relying on Canvas/layer-effect composition; the Bar Media Popup requests 64 samples, the shared CAVA service restarts/re-resolves its source if a running process stops emitting complete frames, and generated raw output explicitly pins `ascii_max_range = 1000` plus the semicolon delimiter expected by the parser. Live play/pause/player-switch/reopen validation is pending.
