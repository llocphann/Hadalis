...
2. **Connected-surface source contract is now placement-driven; live validation is still blocking closure.** Every Bar module popup always joins its Bar-facing edge and automatically joins any Screen Edge its resting body actually reaches after placement/clamping; System Monitor has no module-specific edge opt-in anymore. Attached edges are square and suppress shadow, while free corners retain radius. Shared popup shadow is generated from the same asymmetric rounded body silhouette and clipped at joined edges. OSK and both Sidebars use the same no-stem direct-edge rule, and Overview/dashboard now positions its body directly on the bottom Bar/Screen Edge with no connector geometry. Do not resume unrelated cleanup until the maintainer has live-validated corner/middle Bar placements, top/bottom/left/right orientations, OSK, both Sidebars and Overview.
...
2. keep connected-surface and Weather source frozen except for defects found by live validation; do not reintroduce module-specific edge opt-ins, connector/stem renderers, rectangular side-gradient popup shadows, or the center live Weather clock;
3. hand the exact candidate SHA to the maintainer for `bash scripts/validate-maintainer-local.sh` plus live smoke tests covering Bar module movement (corner vs middle), all Bar orientations, OSK, both Sidebars, Overview/dashboard and the three-panel Weather hover composition;
4. only after those P0 live checks pass, return to Media CAVA/Equalizer runtime validation and remaining Material-only residue. Do not mark release gates complete before that result exists.
