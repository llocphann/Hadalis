...
3. **Maintainer-reported connected-surface/UI issues remain open.** Popup connector geometry, Overview bottom attachment, Media visualizer visibility and Time/Date+Weather hover composition still require concrete source fixes; Sidebar connector ownership is source-fixed but needs live validation; same-edge Screen Edge ownership/color is source-fixed but still needs live validation.
