...
- [x] **Time & Date / Weather hover:** the live Clock no longer creates its separate tooltip; Weather remains the single hover composition, with a Serpantinum-inspired large center clock + 8-hour elliptical forecast around it, responsive compact routing from the real popup output, the calendar retained on the left, and Hadalis-owned detailed weather retained on the right. Live sizing/hover validation is pending.
