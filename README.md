...
- [x] **Fan Control settings placement:** **Fan Control** now lives in the real **Settings → System** page (with System search routing/indexing) and has been removed from Bar Settings entirely. The current backend still only exposes `firmware` / `managed` ownership; per-power-profile fan-level editing remains open until the helper exposes a hardware-safe direct-control capability.
