...
- Screen Edge lower corners: maintainer live-tested `818efdb9` and confirmed the lower inverse-corner geometry now matches the top Bar/Screen Edge reference. The remaining defect is shadow continuity: the vertical Screen Edge shadow stops at the outer edge of the R×R corner footprint while the horizontal shadow already runs beneath the `RoundCorner`, leaving a visible break at each lower corner. The source candidate now continues the missing left/right side-shadow segment inside the horizontal `EdgeWindow` itself, from the vertical-shadow tangent to the bottom band, with the existing `RoundCorner` painted above it. This preserves the validated corner coordinates and avoids relying on compositor ordering between separate layer surfaces. Live lower-left/lower-right shadow continuity validation is still required.
