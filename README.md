...
- [ ] **BLOCKER — Time & Date / Weather hover composition:** keep calendar left + 8-hour orbital forecast center + detailed weather right, but remove the large live clock from the center entirely because it overlaps the orbital forecast. The center should show only the date plus current weather summary. Re-size the right detail card so its outer height aligns visually with the calendar and center components instead of dominating the row. Preserve `Last refresh` inside the right card. Do not resume unrelated cleanup until this and the connected-surface blocker above are source-fixed and handed back for live validation.
...
- [x] **System Monitor content simplification:** the monitor keeps Used/Total RAM plus temperature/load metrics, and ThinkFan is one compact row — `Fan · toggle · Speed · Level` — using the same small body typography as the resource metrics; the source regression now forbids the older two-line `ThinkFan / Fan speed / Fan level` layout. Error feedback remains conditional below the row. Live validation is pending after reloading the updated shell because the maintainer screenshot still showed the older runtime composition.
...
2. **BLOCKER — connected surfaces must follow placement, not module identity.** Every Bar module popup must always join the Bar edge it opens from, and must also automatically join any Screen Edge its body reaches after placement/clamping; e.g. System Monitor near the left edge joins both top Bar + left Screen Edge, but the same module moved to the middle joins only the Bar. The same direct-surface rule applies to OSK, Overview/dashboard and both Sidebars: no connector/stem object is part of the visual contract. Every attached edge must be square/no-radius and must cast no shadow across the joined Bar/Screen Edge; only unattached outer corners remain rounded. Popup shadow must follow the popup silhouette/corner radii and be clipped exactly at every joined edge instead of using rectangular side gradients that can paint over Screen Edge. Treat this blocker as higher priority than Material cleanup.
...
2. fix the shared connected-surface geometry/frame first: automatic Bar + touched-Screen-Edge joins, square attached corners, radius-following shadow clipped at joins; propagate the same no-stem rule to OSK/Overview/Sidebars without rebuilding the retired broad perimeter runtime;
3. fix the Weather hover composition next: remove the center live clock, retain date/current weather summary, and equalize the three outer component heights;
4. only after those blockers are source-fixed, return to Media CAVA/runtime validation and remaining Material-only residue;
5. hand the exact candidate SHA to the maintainer for `bash scripts/validate-maintainer-local.sh` plus the live desktop smoke matrix. Do not mark release gates complete before that result exists.
