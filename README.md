...
### 2.1 Locked ownership contract — square Screen Edge baseline

The maintainer explicitly reset the physical Screen Edge to a square 90° baseline on 2026-09-19 after repeated top/bottom/left/right tests showed that overlapping rounded geometry produced position-dependent corner shapes.

- **Normal Bar mode owns only the Bar body.** Horizontal and vertical Bar runtimes must not paint `roundDecorators`, Screen Edge contact rectangles, normal-mode `barEdgeShadow` rectangles, `RoundCorner` wedges, or any synthetic perimeter extension outside the body.
- **`ScreenEdges.qml` owns a square orthogonal frame only.** It may paint the four straight edge bands plus their straight inward shadows; it must not paint endpoint arcs, inverse corners, radial corner shadows, rounded wedges or radius-dependent host geometry.
- The public `appearance.screenEdge.radius` control/schema/default is retired while this square baseline is active. Screen Edge width and shadow controls remain supported.
- Normal Bar layer-surface size collapses to the body itself. Extra inward host space is allowed only for the straight shadow of the conditional auto-hide Screen Edge fallback.
- `autoHideScreenEdge` is a deliberate exception only for visibility: when the Bar translates completely away, the fallback may draw one **square** edge band and straight shadow. It must not restore rounded corners.
- Repositioning the Bar between top/bottom/left/right must not change Screen Edge corner geometry; every physical corner remains a 90° intersection.
- `scripts/test-shell-surface-contracts.py` is the regression gate for this square baseline. Do not recreate the removed Bar wedges/contact rectangles or Screen Edge corner primitives without a new maintainer-approved redesign.
...
- Latest visual follow-up: the physical Screen Edge/Bar corner experiment has been reset to a square 90° baseline. Radial Screen Edge corner-shadow junctions and rounded endpoint geometry are removed; connected Popup/Sidebar/Dashboard/Settings shoulders remain a separate shared connected-surface concern. Dashboard/TLP/auto-hide follow-ups are otherwise unchanged. Live validation remains required.
...
- Screen Edge corner reset: the previous lower-corner/inverse-corner/radial-shadow experiment is retired. All four physical Screen Edge corners are now square 90° intersections; only straight edge bands and straight inward shadows remain. The former `appearance.screenEdge.radius` control is removed from Settings/schema/defaults. Live top/bottom/left/right validation is required before any future corner redesign.
- Bar/Screen Edge intersection cleanup: repeated top/bottom/left/right tests showed that overlapping Bar and Screen Edge geometry made the corner shape depend on orientation/layer ordering. Bar-owned inverse-corner wedges, Screen Edge contact rectangles and normal-mode Bar shadow rectangles are removed from both Bar runtimes. `ScreenEdges.qml` now owns only a square physical frame, and the auto-hide fallback is square as well. Live four-position validation of this baseline is pending.
