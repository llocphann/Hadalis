...
- Screen Edge corner reconstruction: the rejected `4 EdgeWindow + 4 CornerWindow` composition remains retired. The visible Screen Edge is one full-screen odd-even frame geometry: padded outer rectangle minus a single 25px rounded inner workspace rectangle. The four-corner geometry lock is unchanged; the physical shadow is an effect on that same Shape only, with no additional painted geometry. Live validation of corners and shadow at native and fractional scale is required.
