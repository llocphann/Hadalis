...
- [x] **On-screen keyboard Screen Edge connector:** the draggable OSK now snaps its body to a Screen-Edge-aware top/bottom inset and renders the shared `ConnectedSurfaceConnector` between the keyboard body and the physical edge; the connector hides while dragging and reattaches after the nearest-edge snap. Live drag/pin/top/bottom validation is pending.
...
- [x] **Time & Date / Weather hover:** the live Clock no longer creates or exports its retired tooltip surface; Weather remains the single hover composition, with a Serpantinum-inspired center clock + 8-hour elliptical forecast, responsive compact routing from the real popup output, the calendar retained on the left, and Hadalis-owned detailed weather retained on the right. The center clock is now reduced and shifted slightly upward to avoid crowding, and `Last refresh` lives inside the right-hand detail panel instead of extending the whole popup. Live sizing/hover validation is pending.
- [x] **Left/right Sidebar connectors:** the connector lives inside the owning `SidebarHost` PanelWindow, shares its output/centering/lifecycle, overlaps the physical edge-to-card gap through `PerimeterTokens.seamOverlap`, and uses the sidebar's Material/card surface token. The bridge now renders above the later-declared sidebar body so that overlap actually covers the body's attachment-edge border instead of leaving a one-pixel seam. Live left/right validation is pending.
- [x] **Overview bottom connector:** the dashboard's visible `dashContainer` exposes its exact body rect/color and the owning Overview window draws a shared `ConnectedSurfaceConnector` from that body to the inner boundary of the bottom Screen Edge using the canonical width/seam tokens. The connector renders above the dashboard body so its seam overlap covers the body outline at the joint. Live Meta/Super+Space validation is pending.
- [x] **System Monitor corner composition:** `ResourcesPopup.qml` opts into the shared adjacent-Screen-Edge path, so a System Monitor popup clamped near a Bar corner can join both its Bar edge and the orthogonal persistent Screen Edge without a parallel popup framework. The popup also drops Free RAM, verbose ThinkFan status prose and the redundant Service metric; ThinkFan typography now matches the monitor headers. Live corner/top-left validation is pending.
- [x] **Fan Control settings + guarded power-profile levels:** **Fan Control** lives in the real **Settings → System** page (with System search routing/indexing) and remains removed from Bar Settings. On hardware where ThinkPad ACPI exposes writable direct fan control, the helper/service now supports explicit `Auto` or levels `1–7`, persisted separately for Power Saver/Balanced/Performance. Profile following is opt-in/off by default, waits through startup restore, refuses fixed writes while ThinkFan managed ownership is active, and never rewrites machine-specific ThinkFan sensor/curve configuration.
