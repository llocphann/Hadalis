...
- **Connected bodies keep rounded contact corners.** Caelestia's `PanelBg` remains rounded while its `BlobRect` is unioned with the border inside the shared `BlobGroup`; the earlier Hadalis mistake was squaring the body corner itself and expecting a flare alone to recreate that silhouette. Hadalis now keeps the existing 20px smooth-union shoulder approximation, but the actual Popup/Sidebar/Dashboard/Settings/OSK body corner touching Screen Edge/Bar uses `PerimeterTokens.attachedCornerRadius = frameRadius`. Free corners retain their existing component radius. This changes only connected-surface body geometry and does not modify the locked Screen Edge/Bar renderer.
...
- [ ] **Screen Edge corner radius is configurable in Settings and defines normal ii Bar plus attached connected-body corners.** Default is 25px. Screen Edge and Bar/VerticalBar keep the canonical inverted-frame radius; Popup/Sidebar/Dashboard/Settings/OSK bodies touching that perimeter consume the same value through `PerimeterTokens.attachedCornerRadius`. Smooth-union shoulder width remains independently fixed at 20px.
