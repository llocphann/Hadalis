...
- [x] **Clock Calendar + Weather tabs — source fixed:** Calendar is detached from Weather and opens from Clock through `ClockCalendarPopup`. Weather now keeps one stable 430px (360px compact) × 270px body and overlays orbital forecast/detail as two tabs. A two-dot rail is vertically centered on the right edge; wheel/touchpad vertical input selects the previous/next tab, and the two pages form a clipped vertical stack animated with the shared element-move timing/easing. Live wheel direction, hover retention, slide timing and all Bar orientations remain pending.
