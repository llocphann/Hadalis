...
- Caelestia's border defaults are preserved: Screen Edge thickness defaults to **10px**, inner rounding is fixed at **25px** during validation, and the outer path extends **50px** beyond the window so the compositor clips the physical screen boundary rather than exposing antialiasing on an outer shape edge.
...
- Physical Screen Edge shadow is rendered exactly as an effect on the **same locked inverted frame**, never as extra geometry. The default follows Caelestia ContentWindow exactly: Material `m3shadow`, `blurMax = 15`, `shadowBlur = 1.0`, alpha `0.70`, zero offset. `appearance.screenEdge.shadow.enabled/size/opacity` exposes the restored user controls. Runtime bindings explicitly depend on `Config.revision` so changes written by the standalone Settings process re-evaluate live in the shell. Custom size values use normalized `shadowBlur`; `blurMax` is treated as shader capacity rather than the live amount. None of these controls may alter the four-corner path.
...
- The shared `appearance.screenEdge.shadow` values continue to feed connected popup/Sidebar/Dashboard shadow consumers as before; the physical frame now consumes the same canonical settings again.
- `scripts/test-shell-surface-contracts.py` is the regression gate. Do not restore `CornerWindow`, painted `EdgeWindow`, per-corner paths, gradient/radial shadow geometry or any second physical Screen Edge renderer.
...
- Latest visual follow-up: the physical Screen Edge remains a single locked inverted-frame renderer, including while ii Bar auto-hide is enabled; Bar/VerticalBar still carry no `autoHideScreenEdge` fallback. Its shadow is now restored using the same single-frame `MultiEffect` pattern as Caelestia ContentWindow, with 15px/70% defaults and no extra painted geometry. The Screen Edge Settings card again exposes enable, size and opacity controls. Live validation of shadow clipping/softness at all four corners remains required.
- Bar/popup shoulders and shadow regression: the physical Bar/Screen Edge perimeter no longer uses inverse-corner shoulders or Bar-owned `RoundCorner` geometry; it is square by contract. Connected popup shoulders remain implemented by the shared connected-surface primitives and are independent of the physical Screen Edge baseline. Popup shadows remain live above the host background. Live top/bottom/left/right and rapid open/close validation remains.
...
- Screen Edge corner reconstruction: the rejected `4 EdgeWindow + 4 CornerWindow` composition remains retired. The visible Screen Edge is one full-screen odd-even frame geometry: padded outer rectangle minus a single 25px rounded inner workspace rectangle. The maintainer-approved four-corner silhouette is locked; the restored shadow is only a `MultiEffect` on that existing alpha shape and cannot introduce corner/edge overlays. Live validation of the four corners plus Caelestia-style shadow at native and fractional scale is required.
