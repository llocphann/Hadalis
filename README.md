...
- The outward shoulder tangent radius follows the same user-controlled `appearance.screenEdge.radius` through `PerimeterTokens.joinFlareRadius = frameRadius`. `joinFlareCrossScale` controls only how flat/compressed the shoulder is across the attachment axis. Free/non-contact popup corners keep their existing component radius.
...
> **Latest maintainer direction (2026-09-19):** Calendar is owned by Clock as a dedicated Obsidian-inspired connected popup. Weather is a separate two-tab connected popup: tab 1 is the 8-hour orbital forecast and tab 2 is detailed weather. Two circular indicators sit vertically centered on the popup's left edge; mouse-wheel/touchpad vertical scrolling moves between tabs. Tab content transitions by opacity fade only—no slide/translate animation and no geometry resize between tabs.

- [ ] **Clock hover:** horizontal Clock and the vertical Clock+Date cluster open the dedicated Calendar popup.
- [ ] **Weather tabs:** orbital forecast and detailed weather share one stable popup footprint and switch by wheel or the left-edge dot indicators.
- [ ] **Fade-only transition:** outgoing/incoming Weather tabs animate only opacity; tab switching must not move the popup body or slide content.
...
- [x] **Clock Calendar + Weather tabs — source fixed:** Calendar is detached from Weather and opens from Clock through `ClockCalendarPopup`. Weather now keeps one stable 430px (360px compact) × 270px body and overlays orbital forecast/detail as two tabs. A two-dot rail is vertically centered on the left edge; wheel/touchpad vertical input selects the previous/next tab, and the only tab transition is a cross-fade via two opacity behaviors. Live wheel direction, hover retention, fade timing and all Bar orientations remain pending.
