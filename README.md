...
- **Heavy panel residency:** ii Overview and the Waffle shared Overview host now release their multi-output Overview trees after the configured exit animation plus a small safety grace, and Waffle Action Center releases its hidden `PanelWindow`/media/stack subtree after the shared Waffle exit duration. Wallpaper Launcher now keeps its search query in lightweight `GlobalStates` state while both panel-family hosts release the heavy launcher tree after its real exit animation; current wallpaper/index are still resynchronized on open. Waffle Start Menu and the grid Wallpaper Selector already self-unload their heavy inner windows. Continue the audit with Coverflow and other retained surfaces, preserving user-visible state before removing residency.
