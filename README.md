...
2. **Connected-surface outward contact flares are shared by Popup/Sidebar/Dashboard/Settings/OSK surfaces.** The failed inward-rounded/standalone-`PathArc` experiments stay reverted. `ConnectedSurfaceFrame` plus direct Sidebar/Dashboard/Settings/OSK bodies keep the attached edge square (radius 0); `ConnectedSurfaceJoinFlares` paints outward shoulders outside the body using live mapped geometry and exact owner contact planes. Raster crack prevention is centralized: the mathematical contact plane is never shifted; `PerimeterTokens.seamOverlap` is rendered only as a same-color owner-side overlap and the shared reveal clip permits that overlap during slide-under motion. Settings Overlay/Focus anchor their bottom shoulders to the real bottom Bar/Screen Edge seam instead of full-overlay coordinates. The tangent radius follows `PerimeterTokens.joinFlareRadius = frameRadius`, so the Screen Edge Border Radius setting affects the flare without modifying the locked Screen Edge/Bar renderer. Free body corners retain their existing component radii.
