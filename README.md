...
- Physical Screen Edge shadow is rendered exactly as an effect on the **same locked inverted frame**, never as extra geometry. The default follows Caelestia ContentWindow: Material `m3shadow`, `blurMax = 15`, alpha `0.70`. `appearance.screenEdge.shadow.enabled/size/opacity` exposes the restored user controls; changing them must not alter the four-corner path.
