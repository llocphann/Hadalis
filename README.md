...
2. **Connected-surface body contact corners now inherit the physical perimeter radius.** The failed standalone `PathArc` flare replacement stays reverted. Shared `ConnectedSurfaceJoinFlares` keeps its 20px smoothing approximation, while `ConnectedSurfaceFrame` and direct Sidebar/Dashboard/Settings/OSK bodies round only the corners touching Screen Edge/Bar with `PerimeterTokens.attachedCornerRadius`. Free corners retain their existing radii. Runtime validation is still blocking closure.
