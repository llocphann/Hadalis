...
1. **Material-only active-tree residue is now outside Settings chrome.** `SettingsOverlay.qml` and the standalone `settings.qml` no longer contain legacy `Appearance.*Everywhere` presentation branches. The first internal `Appearance.qml` cleanup group also collapsed blur style gates, top-level hover/active aliases and warning tokens to their Material paths while retaining public aliases for caller compatibility. Continue auditing the larger palette/motion branches and exact non-Settings callers before removing compatibility properties or legacy palette objects.
