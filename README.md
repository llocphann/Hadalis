...
- Screen Edge lower corners: the latest maintainer light-mode capture showed that the explicit bottom Canvas paths still produced triangular wedges. The corner overlays now use one canonical `RoundCorner.CornerEnum.TopLeft` silhouette—the same primitive family as the visibly correct Hug top corners—and rotate the complete square item for top-right/bottom-left/bottom-right. Straight shadows remain inset from free corner footprints. Live four-corner comparison with Screen Edge shadow enabled remains.
