...
- The four thin `ReservationWindow` surfaces are transparent compositor reservations only. They never paint Screen Edge pixels and therefore cannot change the frame silhouette.
- Physical Screen Edge shadow remains retired. The frame must not add gradients, radial shadows, drop shadows or Bar-owned corner geometry.
...
- The legacy `appearance.screenEdge.shadow` data remains temporarily only because connected popup/Sidebar/Dashboard surfaces still consume it for their separate non-physical shadows.
- `scripts/test-shell-surface-contracts.py` is the regression gate. Do not restore `CornerWindow`, painted `EdgeWindow`, per-corner paths or physical Screen Edge shadows.
...
- Screen Edge corner reconstruction: the rejected `4 EdgeWindow + 4 CornerWindow` composition is retired. The visible Screen Edge is now one full-screen odd-even frame geometry: padded outer rectangle minus a single 25px rounded inner workspace rectangle. This removes every painted overlap between corners and straight edges. Physical Screen Edge shadow remains disabled. Live validation of the four corners at native and fractional scale is required.
