...
- [x] **Overview Dashboard as connected popup:** Overview keeps connector/stem geometry retired and positions Dashboard at the exact bottom Bar/Screen Edge boundary. Dashboard remains on the prior shared `ConnectedSurfaceJoinFlares` baseline after the exact Screen Edge-corner experiment was reverted. Dashboard presentation mode now also bypasses generic full-screen Overview desaturation/dim post-processing so its solid Material `colLayer0` owner stays color-aligned with Sidebar and `StyledPopup`; ordinary Search/Task View keeps the existing overlay effect. Reverse close stays mapped through the reveal tail. Live Meta/Super+Space/open-close/search-transition and surface-color parity validation is pending.
