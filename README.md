...
2. **The maintainer's latest screenshots changed the connected-surface visual contract.** Bar/Screen Edge popups, OSK and both Sidebars now use direct edge overlap with no visible connector stem; shared Screen Edge width/shadow owns the boundary. The remaining connector-shaped non-Bar surface is the Overview/dashboard bottom attachment, which still needs conversion to the same direct-edge rule; Media/Weather/Fan fixes remain pending local validation.
