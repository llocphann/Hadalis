...
### 2.1 Locked ownership contract — shadow-free square Screen Edge baseline

The maintainer explicitly reset the physical Screen Edge to a minimal geometry baseline on 2026-09-19 after repeated top/bottom/left/right tests showed that overlapping rounded and shadow geometry produced position-dependent results.
...
- **`ScreenEdges.qml` is the only physical Screen Edge renderer.** In the current baseline it paints exactly four rectangular edge bands. It must not paint shadows, gradients, endpoint arcs, inverse corners, radial corner shadows, rounded wedges or radius-dependent host geometry.
- The public `appearance.screenEdge.radius` and physical Screen Edge shadow controls are retired. Screen Edge width remains the only public physical-edge geometry control during this reconstruction pass.
- When the ii Bar uses auto-hide, it relinquishes physical-edge ownership to `ScreenEdges.qml`. There is no `autoHideScreenEdge` substitute inside Bar/VerticalBar, so hiding the Bar exposes the same Screen Edge implementation that will receive the next Caelestia-style geometry work.
- The legacy `appearance.screenEdge.shadow` data is temporarily retained only because existing connected popup/Sidebar/Dashboard surfaces still consume it for their own non-physical shadows; it no longer renders around the physical Screen Edge and is no longer exposed as a Screen Edge setting.
- Repositioning or hiding the Bar must not change which implementation draws the physical Screen Edge.
- `scripts/test-shell-surface-contracts.py` is the regression gate for this baseline. Do not restore physical Screen Edge shadows or Bar-local fallback geometry without a new maintainer-approved redesign.
...
- Latest visual follow-up: the physical Screen Edge has been reduced further to a shadow-free rectangular baseline. `ScreenEdges.qml` is now the only physical-edge renderer, including while ii Bar auto-hide is enabled; Bar/VerticalBar no longer carry an `autoHideScreenEdge` fallback. Connected Popup/Sidebar/Dashboard/Settings shadows remain a separate connected-surface concern. Live validation remains required.
...
- Screen Edge geometry reset: the previous corner and shadow experiments are retired. All four physical edges are now plain rectangular bands with no inward shadow, gradient, corner primitive or extra host extent. The former radius and physical shadow controls are removed from public Settings. Live all-four-edge validation is required before the Caelestia-style geometry is rebuilt.
