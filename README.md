...
- [x] **Left/right Sidebar edge seam:** the bridge remains inside the owning `SidebarHost` PanelWindow for output/lifecycle ownership, while the left, default-right and compact-right outer surfaces no longer draw their own border. Screen Edge therefore owns the outer shell boundary instead of meeting a second one-pixel outline. Live left/right validation is pending.
...
2. **The maintainer's latest screenshots changed the connected-surface visual contract.** Bar/Screen Edge popups and OSK now use direct edge overlap with no visible connector stem plus the shared Screen Edge shadow, and Sidebar outer borders are removed so Screen Edge owns the seam. Remaining connector-shaped non-Bar surfaces (notably Overview/Sidebar bridge geometry) still need a follow-up audit against this direct-edge rule; Media/Weather/Fan fixes remain pending local validation.
