...
2. **The maintainer's latest live screenshots reopened several runtime-sensitive items.** Screen Edge persistence/shadow now has a source fix, while OSK edge attachment, Sidebar seam/border cleanup, System Monitor dual-edge composition, Media Equalizer visibility, Weather compaction and Fan Control settings placement/profile controls remain active source work until addressed and then locally validated.
