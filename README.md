...
- [x] **Fan Control settings placement:** Bar Settings now exposes a dedicated **System** section and the previous `System Monitor & Thermals` block is renamed **Fan Control** there instead of living under Modules. The current backend still only exposes `firmware` / `managed` ownership; per-profile speed editing remains open until a hardware-safe profile/curve backend contract exists.
...
2. **The maintainer's latest live screenshots reopened several runtime-sensitive items.** Screen Edge persistence/shadow, OSK edge attachment, Sidebar seam stacking, Overview bottom attachment, Media Equalizer rendering, System Monitor dual-edge composition, Weather compaction and Fan Control placement now have source fixes. Fan-speed editing per profile remains unfinished because the current ThinkFan bridge has no hardware-safe profile/curve editing contract; do not fake that UI or overwrite machine-specific ThinkFan configuration.
