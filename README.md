...
- [x] **Direct Bar/Screen Edge popup composition:** shared `StyledPopup` keeps source-aware tangent placement but now sets `connectorLength: 0`, overlaps the Bar by the shared seam token, and—when corner-clamped—places the body directly on the inner Screen Edge boundary instead of drawing a second connector stem. Popup free sides use the same Screen Edge shadow enable/size/opacity settings, while attached sides suppress shadow to avoid a seam. This follows Caelestia's single-surface composition principle within Hadalis' existing popup architecture. Live top/bottom/left/right validation is pending.
...
- [x] **Time & Date / Weather hover:** Weather remains the single hover composition with calendar left, 8-hour orbital forecast/clock center and Hadalis detailed weather right. The center clock is reduced again to 2.0× the large text token, the orbital cluster is biased upward, and its panel height is cut from 350px to 300px so the previous empty bottom band no longer dictates popup height. `Last refresh` remains inside the right detail panel. Live sizing/hover validation is pending.
...
- [x] **System Monitor content simplification:** the monitor keeps Used/Total RAM plus temperature/load metrics, and ThinkFan is now one compact row — `Fan · toggle · Speed · Level` — using the same small body typography as the resource metrics. Error feedback remains conditional below the row. The popup edge-composition itself is being revised separately to the maintainer's newer direct-edge Caelestia contract. Live validation is pending.
- [x] **Fan Control settings + durable power-profile levels:** **Fan Control** lives in the real **Settings → System** page (with System search routing/indexing) and remains removed from Bar Settings. Power Saver/Balanced/Performance level preferences now persist independently of runtime helper readiness, use an explicit serialized Config flush, and are owned by `ThinkFanService` rather than duplicated Settings-side logic. The shell root keeps the service alive so profile following continues after Settings/System Monitor closes; busy helper operations queue the latest requested level, leaving ThinkFan managed mode reapplies the active profile level, and stale installed helpers are detected through a fixed-level capability marker. Runtime apply still requires the repo/package-managed `/usr/libexec/inir-thinkfan` bridge to be current and ThinkPad ACPI direct control to be available; no machine-specific ThinkFan sensor/curve configuration is rewritten.
...
2. **The maintainer's latest screenshots changed the connected-surface visual contract.** Bar/Screen Edge popup composition and OSK now use direct edge overlap with no visible connector stem plus the shared Screen Edge shadow. Sidebar outer-border cleanup and any remaining standalone connector-shaped surfaces still require source audit before the next live pass; Media/Weather/Fan fixes remain pending local validation.
