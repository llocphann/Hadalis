...
- [x] **Time & Date / Weather hover:** Weather remains the single hover composition with calendar left, 8-hour orbital forecast/clock center and Hadalis detailed weather right. The center clock is now 1.55× the large text token, the orbital cluster is shifted farther upward, the center panel is reduced to 270px, its bottom margin is removed, and the root height is pinned to the composition implicit height so stale vertical slack cannot enlarge the popup. `Last refresh` remains inside the right detail panel. Live sizing/hover validation is pending.
...
2. **The maintainer's latest screenshots changed the connected-surface visual contract.** Bar/Screen Edge popups, OSK and both Sidebars now use direct edge overlap with no visible connector stem; popup corners touching an orthogonal Screen Edge are squared so the body reads as one continuous edge surface, while only the free popup sides keep the shared Screen Edge shadow. The remaining connector-shaped non-Bar surface is the Overview/dashboard bottom attachment, which still needs conversion to the same direct-edge rule; Media/Weather/Fan fixes remain pending local validation.
