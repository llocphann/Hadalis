...
- **Dashboard/Overview:** the first warm-open attempt was runtime-rejected and reverted. The replacement keeps the standalone Dashboard layer-shell window mapped after first use, empties its input region and suspends rendering while closed, keeps the widget tree resident, and paints one closed frame before the fixed 24px/300ms `SurfaceMotion` slide. This specifically avoids compositor map/unmap effects masquerading as a fade. Live-validate long-idle reopen behavior, plus SongRec geometry cleanup, Dashboard <-> Applications fade-through, compact System Monitor readability, horizontal Available Modules overflow and Niri Overview drag/drop/focus stability.
...
7. **Dashboard/Overview:** live-validate the mapped warm Dashboard lifecycle: after the first open, waiting/closing/reopening must not rebuild the widget tree; the native surface must stay mapped with empty input/render sleep while closed; every reopen must visibly use the 24px/300ms slide with no opacity/scale fade. Also visually accept SongRec geometry cleanup, Dashboard <-> Search Applications crossfade, the redesigned System Monitor, horizontal Available Modules overflow, and Niri Overview drag/drop/focus stability.
...
7. Live-validate Dashboard mapped warm reopen: sau lần mở đầu tiên, native surface vẫn mapped nhưng input rỗng/render sleep khi đóng, widget tree không rebuild, và mỗi lần mở lại phải thấy rõ slide 24px/300ms không opacity/scale fade; đồng thời accept SongRec geometry, Dashboard <-> Search Applications crossfade, System Monitor layout, Available Modules horizontal overflow và Overview drag/drop.
