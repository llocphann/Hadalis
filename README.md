...
- The contact blend radius follows the same user-controlled `appearance.screenEdge.radius` through `PerimeterTokens.joinFlareRadius = frameRadius`. Free/non-contact popup corners keep their existing component radius; there is no second cross-axis/ellipse tuning token.
...
- Prefer one shared `ConnectedSurfaceFrame` + `ConnectedSurfaceJoinFlares` path used by `StyledPopup`, Sidebar, Dashboard, Settings surfaces and OSK. Do not create per-popup patches or a second popup framework.
...
- Caelestia's real implementation uses `BlobRect` surfaces in the same `BlobGroup` as `BlobInvertedRect`; its shader merges the SDFs with a circular smooth-min. Hadalis cannot share that SDF field across separate layer-shell windows, so `ConnectedSurfaceContactFlare.qml` reconstructs the local `smin = 0` excess union as one canonical inverse quarter-circle and `ConnectedSurfaceJoinFlares.qml` only mirror/rotates it for top/bottom/left/right endpoints. Do not replace it with inward body rounding, per-orientation Beziers, or a second Screen Edge renderer.
...
- Screen Edge may keep its own fullscreen mapping policy; do not infer that Bar must mirror it. Bar and Screen Edge have shared corner geometry but different native-surface lifecycle requirements.
...
- **Connected contact corners flare outward; attached body corners stay square.** Caelestia unions `PanelBg`/popout blobs with the border inside one smoothed `BlobGroup`, which visually produces a broad outward shoulder. Hadalis approximates that on separate layer-shell surfaces with `ConnectedSurfaceJoinFlares`: the attached body corner remains radius 0, while a fill-only outside shoulder uses `joinFlareRadius = frameRadius` plus `joinFlareCrossScale` to create the flattened transition. Do not reintroduce `attachedCornerRadius` or inward rounding on the attached body edge. Free corners retain their existing component radius.
...
- [ ] **Bar survives fullscreen enter/exit without reload.** Horizontal and vertical ii Bar native surfaces stay mapped/updating; fullscreen coverage is owned by compositor stacking, not `visible`/`updatesEnabled` gates. Leaving fullscreen must restore all Bar contents immediately without `inir restart` or shell reload.
...
- [ ] **Screen Edge corner radius is configurable in Settings and defines normal ii Bar plus connected outward shoulders.** Default is 25px. Screen Edge and Bar/VerticalBar keep the canonical inverted-frame radius; Popup/Sidebar/Dashboard/Settings/OSK outward flare tangent width follows the same value through `PerimeterTokens.joinFlareRadius = frameRadius`. Attached body corners remain square; `joinFlareCrossScale` controls only shoulder flattening depth.
...
- [x] **Overview Dashboard as connected popup:** Overview keeps connector/stem geometry retired and positions Dashboard at the exact bottom Bar/Screen Edge boundary. Dashboard remains on the prior shared `ConnectedSurfaceJoinFlares` baseline after the exact Screen Edge-corner experiment was reverted. Reverse close stays mapped through the reveal tail. Live Meta/Super+Space/open-close/search-transition validation is pending.
...
2. **Connected-surface outward contact flares are source-complete but still require runtime validation.** The failed inward-rounded/standalone-`PathArc` experiments stay reverted. `ConnectedSurfaceFrame` and direct Sidebar/Dashboard/Settings/OSK bodies keep the attached edge square (radius 0). The shared `ConnectedSurfaceContactFlare` now renders the circular-smooth-union zero-isocontour derived from Caelestia's BlobGroup shader, while `ConnectedSurfaceJoinFlares` only positions/mirror-rotates that one primitive for all eight endpoints. `PerimeterTokens.joinFlareRadius = frameRadius` keeps the contact silhouette reactive to the Screen Edge Border Radius setting without modifying the locked Screen Edge/Bar renderer. Free body corners retain their existing component radii.
