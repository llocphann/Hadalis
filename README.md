...
### 2.1 Locked ownership contract — one perimeter owner

The maintainer explicitly approved a corner-ownership redesign on 2026-09-19 after repeated top/bottom/left/right runtime tests showed that overlapping Bar and Screen Edge geometry produced position-dependent corner shapes.

- **Normal Bar mode owns only the Bar body.** Horizontal and vertical Bar runtimes must not paint `roundDecorators`, Screen Edge contact rectangles, normal-mode `barEdgeShadow` rectangles, or any other synthetic perimeter extension outside the body.
- `ScreenEdges.qml` is the sole persistent owner of the physical perimeter and therefore the only place where the final Caelestia-style inner Bar/Screen Edge corner geometry should be added.
- Normal Bar layer-surface size must collapse to the body itself. Extra inward host space is allowed only while the conditional auto-hide Screen Edge fallback is active.
- `autoHideScreenEdge` is a deliberate exception: when the Bar translates completely away, that temporary fallback may draw its own edge band/shadow/corners until the Bar returns.
- The free Screen Edge endpoint curves remain owned by `ScreenEdges.qml` and continue to use the single `appearance.screenEdge.radius` setting. Do not create a second Bar-specific radius.
- Repositioning the Bar between top/bottom/left/right must not leave geometry whose ownership depends on layer-surface ordering or stale overlap from the previous orientation.
- `scripts/test-shell-surface-contracts.py` is the regression gate for this ownership baseline. The next inner-corner implementation must extend `ScreenEdges.qml`; it must not recreate the removed Bar wedges/contact rectangles.
...
- Latest five-item visual follow-up: source now uses radial corner-shadow junctions for Screen Edge/Bar, adds shadowed concave shoulders across connected Popup/Sidebar/Dashboard/Settings surfaces, disables Dashboard scrim/wallpaper tint while keeping the shared Dashboard Equalizer DSP, reduces TLP categories to two rows of five with Battery care merged into the main Battery/TLP card and scoped left-aligned category labels, and keeps a Screen Edge fallback visible under an auto-hidden Bar. Live validation of all five remains required.
...
- Bar/Screen Edge intersection cleanup: the latest repeated top/bottom/left/right tests showed that the Bar/Screen Edge junction was not stable under orientation changes because normal Bar mode and persistent Screen Edge mode both contributed geometry at the same physical intersection. The previous Bar-owned inverse-corner wedges, Screen Edge contact rectangles and normal-mode Bar shadow rectangle have now been removed from both horizontal and vertical Bar runtimes. Their normal layer surfaces collapse to body size; only the auto-hide fallback may retain temporary perimeter geometry. `ScreenEdges.qml` now has the single persistent ownership baseline required for the next Caelestia-style inner-corner implementation. Live four-position validation of this cleanup baseline is pending.
