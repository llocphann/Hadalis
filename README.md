...
- [x] **Shared popup slide reveal:** `ConnectedSurfaceGeometry` no longer shrinks the body from connector width/zero depth. It preserves the resting body size and translates it toward/from the owning top/bottom/left/right edge using `PerimeterTokens.revealSlideDistance`; `StyledPopup` content follows the same reveal progress. Waffle inherits the same primitive. Live Caelestia-style motion validation is pending.
...
- [x] **Left/right Sidebar direct Screen Edge attachment:** the owning SidebarHost is still a physical left/right layer surface with no connector stem, but the visible body now underlaps the full Screen Edge band all the way to the physical display edge instead of stopping at `screenEdge.width - seamOverlap`. The body grows only toward the attached edge, so its inward/free edge and effective content extent do not move. Left/default-right/compact-right surfaces keep the attached-side border disabled. Live left/right/custom-edge-width validation is pending.
