...
- Left Sidebar Music: the user-facing YT Music tab and YT-specific Settings controls are replaced source-side by a local-only MPD frontend. Songs, MPD saved playlists, folder collections and the live MPD queue are exposed as Songs / Playlists / Queue; normal transport uses mpd-mpris/MPRIS while queue replacement/database update use MPD protocol. The default localhost:6600 endpoint retains the distro `mpd-mpris.service`; a configured non-default host/port or Unix socket now launches a shell-owned `org.mpris.MediaPlayer2.mpd.hadalis` bridge bound to that exact endpoint, preventing transport from silently controlling a different MPD server. Settings exposes the MPD endpoint plus an optional local music-root override for artwork. Canonical defaults, welcome profiles and the Sidebar layout editor use the `music` tab id; legacy `ytmusic` order values are normalized, and the retired YT Music notification/fullscreen search entries are no longer published by Settings. The superseded private mpv IPC helper and standalone filesystem scanner are removed so MPD is the only local library/queue owner. Live MPD database, playlist, queue and MPRIS bridge behavior still require maintainer validation.
