...
- **Dashboard/Overview:** source now keeps the standalone Dashboard warm after first use and arms each open only after a real closed frame, so long-idle reopens should reuse the existing widget tree and enter with the immutable slide instead of rebuilding into a compositor-like fade. Live-validate that behavior, plus SongRec geometry cleanup, Dashboard <-> Applications fade-through, compact System Monitor readability, horizontal Available Modules overflow and Niri Overview drag/drop/focus stability.
...
7. Live-validate Dashboard warm reopen: sau lần mở đầu tiên, đóng/chờ/mở lại không rebuild widget tree và luôn chạy slide entrance; đồng thời accept SongRec geometry, Dashboard <-> Search Applications crossfade, System Monitor layout, Available Modules horizontal overflow và Overview drag/drop.
