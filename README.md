...
- **Heavy panel residency:** ii Overview and the Waffle shared Overview host now release their multi-output Overview trees after the configured exit animation plus a small safety grace, and Waffle Action Center releases its hidden `PanelWindow`/media/stack subtree after the shared Waffle exit duration. Waffle Start Menu and the grid Wallpaper Selector already self-unload their heavy inner windows, so they are lower-priority outer-retention targets. Continue the audit with Wallpaper Launcher/Coverflow, preserving any user-visible state outside the expensive view before removing retention.
