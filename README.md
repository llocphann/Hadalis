...
### 2.1 Locked ownership contract — Caelestia-style Screen Edge corners

The maintainer reset the physical Screen Edge to one renderer and then approved rebuilding the four inner corners to match the Caelestia border silhouette.
...
- **`ScreenEdges.qml` is the only physical Screen Edge renderer.** It owns four straight edge bands plus four inner-corner surfaces.
- All four inner corners use **one canonical top-left Shape/PathArc primitive** mirrored horizontally/vertically for top-right, bottom-left and bottom-right. This prevents orientation-specific geometry drift.
- The reconstruction uses Caelestia's default border values: Screen Edge thickness remains configurable with a 10px default, while the inner-corner radius is temporarily fixed at **25px** until live validation is complete.
- Physical Screen Edge shadow remains retired. The corner renderer must not add gradients, radial shadows, drop shadows or extra Bar-owned geometry.
- When the ii Bar uses auto-hide, it relinquishes physical-edge ownership to `ScreenEdges.qml`. There is no `autoHideScreenEdge` substitute inside Bar/VerticalBar, so hiding the Bar exposes this exact four-edge/four-corner implementation.
- The legacy `appearance.screenEdge.shadow` data is temporarily retained only because existing connected popup/Sidebar/Dashboard surfaces still consume it for their own non-physical shadows; it does not render around the physical Screen Edge.
- `scripts/test-shell-surface-contracts.py` is the regression gate. Do not create separate per-corner implementations or restore physical Screen Edge shadows/Bar-local fallback geometry.
...
- Screen Edge corner reconstruction: the shadow/overlap experiments remain retired, but the four inner corners are now rebuilt from one antialiased Qt Quick Shape. A canonical top-left L-shaped frame path uses a 25px quarter-circle inner arc and is mirrored for the other three corners, so all four silhouettes are mathematically identical. Physical Screen Edge shadow remains disabled. Live validation of all four corners at fractional scale is required.
