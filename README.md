...
- [x] **Waffle shared BarPopup connected presentation:** Waffle keeps its own `BarPopup` API/palette and remains a separate supported panel family, but the popup implementation no longer uses compositor `PopupWindow` + visual-margin gap geometry. It now reuses `ConnectedSurfaceGeometry/Frame/ContentHost/Mask`, direct Bar-edge overlap, Caelestia-style concave shoulders, shaped input and same-output Niri backdrop; Waffle callers retain `close()`, `grabFocus()`, `updateAnchor()`, hover-delay and focus-cleared contracts. `visualMargin` now controls free-side shadow extent instead of creating separation. Live Start/Tray/Updates/context-menu validation is pending.
