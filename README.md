...
- Bar/Screen Edge intersection underlay: the four-position light/dark runtime matrix showed the Bar-owned inverse-corner geometry itself is correct; the visible grey/black wedge comes from the perpendicular `ScreenEdges.qml` straight shadow continuing underneath the transparent workspace side of the Bar's `RoundCorner`. Source now gives the Material ii Bar exclusive shadow ownership across its body + inverse-corner span (`barHeight/verticalBarWidth + screenEdge.radius`) while preserving the solid Screen Edge band and all four locked corner anchors. Waffle keeps its separate taskbar intersection behavior. Live top/bottom/left/right validation is pending.
