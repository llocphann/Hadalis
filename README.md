...
- **Connected contact corners reuse the same Screen Edge radius and exact `PathArc` quarter-circle.** `ConnectedSurfaceJoinFlares.qml` keeps one canonical inverse top-left arc and mirrors it for the other orientations. StyledPopup, left/right Sidebar, Dashboard, Settings surfaces and OSK all consume `PerimeterTokens.joinFlareRadius`, which is the same `appearance.screenEdge.radius` value used by the physical frame. No Bezier/ellipse approximation or independent contact-radius token is allowed.
...
- `scripts/test-shell-surface-contracts.py` is the regression gate. Do not restore `CornerWindow`, painted `EdgeWindow`, per-corner paths, separate physical shadow geometry or shared shadow ownership.
...
- [ ] **Screen Edge corner radius is configurable in Settings and shared by connected contact corners.** Default is 25px; the physical frame and Popup/Sidebar/Dashboard/Settings/OSK inverse contact corners must update from the same canonical value.
...
- Bar/popup contact-corner follow-up: the physical Screen Edge remains the single inverted-frame owner, while connected Popup/Sidebar/Dashboard/Settings/OSK endpoints now clone its exact inverse quarter-circle through the shared `ConnectedSurfaceJoinFlares` PathArc primitive. The same configurable `appearance.screenEdge.radius` drives physical corners and contact corners; popup shadows remain independently owned. Live top/bottom/left/right and rapid open/close validation remains.
...
- Screen Edge corner reconstruction: the rejected `4 EdgeWindow + 4 CornerWindow` composition remains retired. The visible Screen Edge is one full-screen odd-even frame geometry: padded outer rectangle minus one rounded inner workspace rectangle. The geometry primitive remains locked, but its radius is now intentionally configurable (25px default, 0–96px) and is the canonical radius consumed by every connected contact corner. The physical shadow remains an effect on that same Shape only. Live validation at native and fractional scale is required.
...
- [x] **Direct Bar/Screen Edge popup composition:** shared `StyledPopup` keeps source-aware tangent placement but sets `connectorLength: 0`, overlaps the Bar by the shared seam token, and—when corner-clamped—places the body directly on the inner Screen Edge boundary instead of drawing a second connector stem. Popup free sides use the connected-surface shadow token, while attached sides suppress shadow to avoid a seam. The shared frame now also fills a token-sized pair of concave endpoint shoulders on each joined edge, approximating Caelestia's smooth blob-union “góc bè” while the popup body itself remains the direct attachment. Double-joined corners suppress the redundant shoulder. Live top/bottom/left/right validation is pending.
...
- [x] **Screen Edge width/radius + shadow ownership + window-gap boundary:** `appearance.screenEdge` is a typed Config/JsonAdapter object with persistent width, shared corner radius, connected-surface shadow settings and a separate physical-frame shadow owner. Persistent Screen Edge reservations still reserve exactly the solid thickness, so Niri Window Gap starts at the inner Screen Edge/Bar boundary. The shared radius defaults to 25px and now drives both the physical frame and connected inverse contact corners. Live width/radius/shadow/gap validation is pending.
...
- [x] **Left/right Sidebar direct Screen Edge attachment:** the owning SidebarHost is still a physical left/right layer surface with no connector stem, and the visible body underlaps the full Screen Edge band all the way to the physical display edge. The host reserves vertical decoration room equal to the shared Screen Edge radius and renders exact mirrored Screen Edge inverse corners at the upper/lower endpoints. The shared corner primitive maps the nested body into host coordinates with `mapToItem()`; SidebarHost therefore does not apply Loader slide/drop translation a second time. Live left/right/custom-edge-width/radius/animation validation is pending.
- [x] **Overview Dashboard as connected popup:** Overview keeps connector/stem geometry retired and positions Dashboard at the exact bottom Bar/Screen Edge boundary. Dashboard owns a dedicated full-body reveal progress behind its clipped resting boundary. `ConnectedSurfaceJoinFlares` now supplies both bottom contacts using the exact configurable Screen Edge inverse quarter-circle, while the joined body corners stay square and `StyledRectangularShadow` suppresses shadow on the attached side. Reverse close stays mapped through the reveal tail. Live Meta/Super+Space/open-close/search-transition/radius validation is pending.
...
2. **Connected-surface source contract is placement-driven; live validation is still blocking closure.** Every Bar module popup always joins its Bar-facing edge and automatically joins any Screen Edge its resting body reaches after placement/clamping. Attached body corners stay square and suppress attached-side shadow; the transition into Bar/Screen Edge is supplied by exact inverse quarter-circle clones of the physical Screen Edge, all driven by the same `appearance.screenEdge.radius`. Free corners retain popup radius. OSK, Sidebars, Dashboard and Settings use the same contact primitive. System Tray menus, ii Bar/Vertical Bar context menus and horizontal/vertical Media popouts consume the shared connected-popup path. Waffle keeps its separate `BarPopup` entrypoint but consumes the same connected-surface primitives. Generic non-Bar `ContextMenu` remains detached by design.
