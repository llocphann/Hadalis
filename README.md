...
- **Heavy panel residency:** ii Overview and the Waffle shared Overview host now release their multi-output Overview trees after the configured exit animation plus a small safety grace, Waffle Action Center releases its hidden `PanelWindow`/media/stack subtree after the shared Waffle exit duration, and ii Clipboard releases its hidden `PanelWindow` plus filtered `ListModel` immediately after the connected-surface exit completes. Wallpaper Launcher keeps its search query in lightweight `GlobalStates` state while both panel-family hosts release the heavy launcher tree after its real exit animation; Coverflow already self-unloads its expensive fullscreen view after its 450 ms exit lifecycle, and both outer hosts now release the remaining scope immediately afterwards instead of retaining it for five minutes. Waffle Start Menu and the grid Wallpaper Selector already self-unload their heavy inner windows. Continue the audit only where retained state or resources are proven meaningful.
- **Recorder polling lifecycle:** global external `wf-recorder` detection keeps the existing slow 15s/30s safety probe when no recorder UI needs immediacy. Visible/pinned ii Recorder controls and the open Waffle Widgets recorder action temporarily request 1s reconciliation, while the existing bounded 350 ms action quick-check remains the fastest path after shell-issued start/stop commands. This preserves external-recorder correctness without restoring permanent fast polling.
