...
void IccScreencopyContext::ext_image_copy_capture_frame_v1_ready() {
	...
	const auto totalArea =
	    static_cast<qreal>(this->request.width) * static_cast<qreal>(this->request.height);
	if (this->capturedFrames == 0 || totalArea <= 0.0) {
		// The first frame establishes the baseline. Treating its full repaint as
		// motion would promote every newly opened preview to live.
		this->mFrameActivity = 0.0;
	} else {
		...
		const auto changed = this->damage.intersected(bounds);
		const auto changedArea =
		    static_cast<qreal>(changed.width()) * static_cast<qreal>(changed.height());
		const auto ratio = totalArea > 0.0 ? changedArea / totalArea : 0.0;
		this->mFrameActivity = ratio < 0.0 ? 0.0 : (ratio > 1.0 ? 1.0 : ratio);
	}
	++this->capturedFrames;
...
IccForeignToplevelSourceManager::IccForeignToplevelSourceManager()
    : QWaylandClientExtensionTemplate(1) {
	this->initialize();
}

IccForeignToplevelSourceManager* IccForeignToplevelSourceManager::instance() {
	static auto* instance = new IccForeignToplevelSourceManager();
	return instance;
}

::ext_image_capture_source_v1*
IccForeignToplevelSourceManager::createSource(::ext_foreign_toplevel_handle_v1* handle) {
	if (!this->isActive() || handle == nullptr) return nullptr;
	return this->create_source(handle);
}

class IccForeignToplevelHandle: public QtWayland::ext_foreign_toplevel_handle_v1 {
public:
	IccForeignToplevelHandle(
	    ::ext_foreign_toplevel_handle_v1* handle,
	    IccForeignToplevelManager* manager
	)
	    : manager(manager) {
		this->init(handle);
	}
	...
	[[nodiscard]] const QString& identifier() const { return this->mIdentifier; }
...
protected:
	void ext_foreign_toplevel_handle_v1_identifier(const QString& identifier) override {
		...
		this->manager->handleUpdated();
	}

	void ext_foreign_toplevel_handle_v1_closed() override {
		if (this->object()) this->destroy();
		this->manager->handleClosed(this);
		delete this;
	}
...
IccForeignToplevelManager* IccForeignToplevelManager::instance() {
	static auto* instance = new IccForeignToplevelManager();
	return instance;
}

IccForeignToplevelHandle*
IccForeignToplevelManager::findHandle(const QString& identifier) const {
	if (identifier.isEmpty()) return nullptr;
	for (auto* handle: this->mToplevels) {
		if (handle->identifier() == identifier) return handle;
	}
	return nullptr;
}

bool IccForeignToplevelManager::canCapture(const QString& identifier) const {
	return this->isActive()
	    && IccManager::instance()->isActive()
	    && IccForeignToplevelSourceManager::instance()->isActive()
	    && this->findHandle(identifier) != nullptr;
}

ScreencopyContext*
IccForeignToplevelManager::captureToplevel(const QString& identifier, bool paintCursors) {
	auto* handle = this->findHandle(identifier);
	if (handle == nullptr || !this->canCapture(identifier)) return nullptr;

	auto* source =
	    IccForeignToplevelSourceManager::instance()->createSource(handle->object());
	if (source == nullptr) return nullptr;

	return IccManager::instance()->createSession(source, paintCursors);
}

void IccForeignToplevelManager::handleUpdated() { emit this->toplevelsChanged(); }

void IccForeignToplevelManager::handleClosed(IccForeignToplevelHandle* handle) {
	this->mToplevels.removeOne(handle);
	emit this->toplevelsChanged();
}

void IccForeignToplevelManager::ext_foreign_toplevel_list_v1_toplevel(
    ::ext_foreign_toplevel_handle_v1* toplevel
) {
	auto* handle = new IccForeignToplevelHandle(toplevel, this);
	this->mToplevels.push_back(handle);
	emit this->toplevelsChanged();
}

void IccForeignToplevelManager::ext_foreign_toplevel_list_v1_finished() {
	for (auto* handle: this->mToplevels) delete handle;
	this->mToplevels.clear();
	if (this->object()) this->destroy();
	emit this->toplevelsChanged();
}
