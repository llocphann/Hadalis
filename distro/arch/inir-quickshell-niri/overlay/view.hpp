...
class ScreencopyView: public QQuickItem {
	...
	/// Approximate fraction of the source damaged since the previous captured frame.
	/// This is zero for capture backends that do not expose compositor damage.
	Q_PROPERTY(qreal frameActivity READ default NOTIFY frameActivityChanged BINDABLE bindableFrameActivity);
...
public:
	...
	[[nodiscard]] QBindable<qreal> bindableFrameActivity() { return &this->bFrameActivity; }
	[[nodiscard]] QBindable<QSizeF> bindableConstraintSize() { return &this->bConstraintSize; }

signals:
	/// Emitted after every successfully captured frame, even if frameActivity is unchanged.
	void frameCaptured();
...
private:
	...
	Q_OBJECT_BINDABLE_PROPERTY(ScreencopyView, qreal, bFrameActivity, &ScreencopyView::frameActivityChanged);
