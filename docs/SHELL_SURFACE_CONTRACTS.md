...
- `separatePinnedFromRunning=false` is one continuous visual list with no separator; the separator exists only in explicit separated mode. Pin/unpin identity matching is case-insensitive so desktop-id casing cannot create duplicate pinned entries.
