...
// Production wrapper around the exact iRiS v2.31.0 field shader.
// Upstream: snowarch/iNiR@9574fa424c0d1008e927454e933a7fbe292f9fb2
// Source/QSB: modules/iris/field/IrisField.frag(.qsb), GPL-3.0.
//
// All shapes remain in full output-local coordinates. paintBounds limits only
// the raster viewport, which lets an Overlay popup include Top-layer owner
// records in SDF join math without repainting those owners.
Item {
    id: root
    ...
    property real smoothing: PerimeterTokens.irisFuseDepth
    ...
    ShaderEffect {
        ...
        readonly property color rim: root.rimColor
        readonly property vector4d edge: Qt.vector4d(
            Math.max(0, root.rimWidth), root.rimWidth > 0 ? 1 : 0, 0, 0)
