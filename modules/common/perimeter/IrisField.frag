...
// Exact concave quarter-circle connecting a popup's free edge to an
// orthogonal tangent Screen Edge.
//
// The result is only the local fillet patch. It does not repaint the owner,
// does not create a helper rectangle, and does not alter the primary Bar weld.
    ...
    float radius)
{
    ...
    // Tangent Screen Edge must be orthogonal to the primary owner.
    ...
    if (primaryHorizontal) {
        // Horizontal Bar owner + vertical tangent Screen Edge.
        bool primaryTop =
        ...
        float tangentBoundary = tangentRight
            ? tangentOwner.x - tangentOwner.z
            : tangentOwner.x + tangentOwner.z;

        float freeBoundary = primaryTop
            ? body.y + body.w
            : body.y - body.w;

        // u = distance inward from Screen Edge.
        u = tangentRight
            ? tangentBoundary - p.x
            : p.x - tangentBoundary;

        // v = distance outward beyond the popup free edge.
        v = primaryTop
            ? p.y - freeBoundary
            : freeBoundary - p.y;
    } else {
        // Vertical Bar owner + horizontal tangent Screen Edge.
        bool primaryLeft =
        ...
        float tangentBoundary = tangentBottom
            ? tangentOwner.y - tangentOwner.w
            : tangentOwner.y + tangentOwner.w;

        float freeBoundary = primaryLeft
            ? body.x + body.z
            : body.x - body.z;
    ...
    // Local r×r corner square.
    //
    // q=(0,0) is the popup/Screen-Edge intersection.
    // q=(r,0) and q=(0,r) are the two tangent points.
    vec2 q = vec2(u, v);

    float squareDistance = roundedBox(
        q,
        vec2(r * 0.5),
        vec2(r * 0.5),
        0.0);

    // Missing quadrant is the inside of this circle.
    // Negating its SDF gives material OUTSIDE the circle.
    float outsideCircle =
        r - length(q - vec2(r, r));
    ...
    // ∩ outside of circle
    //
    // This is an exact quarter-circle concave fillet.
    return max(squareDistance, outsideCircle);
}
...
void main() {
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (also < -0.5 || also > 0.5) {
            ...
            if (other < FAR) {
                bool usedAnalyticFillet = false;

                // Styled connected surfaces encode:
                //   first relation  = primary owner
                //   second relation = tangent Screen Edge.
                //
                // Only apply the analytic operator when both owners are
                // explicit shapes. Other iRiS relation types retain the
                // existing smoothUnion behavior.
                if (join > 0.5 && also > 0.5) {
                    ...
                    if (fillet < FAR) {
                        ...
                        usedAnalyticFillet = true;
                    }
                }

                if (!usedAnalyticFillet) {
                    united = min(
                        united,
                        smoothUnion(
                            other,
