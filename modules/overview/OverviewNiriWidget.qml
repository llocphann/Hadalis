            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    Rectangle {
                        ...
                        // Patched Quickshell exposes Niri's exact foreign-toplevel
                        // identifier as an ICC source. Load by URL so stock
                        // Quickshell never parses the extension-only type and can
                        // continue using the PNG snapshot underneath.
                        Loader {
                            ...
                            source: active ? "NiriAdaptiveWindowPreview.qml" : ""

                            onLoaded: {
                                const preview = item
                                if (!preview)
                                    return
                                preview.windowId = Qt.binding(function() { return windowItem.windowId })
                                preview.windowData = Qt.binding(function() { return windowItem.windowData })
                                preview.presentationActive = Qt.binding(function() { return root.presentationActive })
                                preview.showPreviews = Qt.binding(function() { return parent.showPreviews })
                                preview.hovered = Qt.binding(function() { return windowItem.hovered })
                                preview.focused = Qt.binding(function() { return windowItem.isFocused })
                            }
                        }
