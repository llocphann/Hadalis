...
PanelWindow {
    ...
    WlrLayershell.namespace: "hadalis:perimeter-thinkfan"
    ...
    onRouteOwnedChanged: {
        ...
        Qt.callLater(() => {
            if (!root.routeOwned)
                return
            thinkFanContent.focusInitialControl()
    ...
    ConnectedSurfaceGeometry {
        ...
        bodySize: Qt.size(
            Math.max(340, thinkFanContent.implicitWidth + 24),
            Math.max(220, thinkFanContent.implicitHeight + 24))
    ...
    ConnectedSurfaceContentHost {
        ...
        Flickable {
            ...
            anchors.fill: parent
            clip: true
            contentWidth: Math.max(width, thinkFanContent.implicitWidth)
            contentHeight: Math.max(height, thinkFanContent.implicitHeight)
            interactive: contentWidth > width + 0.5 || contentHeight > height + 0.5
            boundsBehavior: Flickable.StopAtBounds

            ThinkFanPopupContent {
                id: thinkFanContent
                x: Math.max(0,
                    (thinkFanViewport.width - implicitWidth) / 2)
                y: Math.max(0,
                    (thinkFanViewport.height - implicitHeight) / 2)
            }
        }
    }
