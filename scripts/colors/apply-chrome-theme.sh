...
fix_preferences() {
  ...
  # Reconcile the required defaults in one jq pass. When Chrome already has
  # exactly the desired fields, emit no replacement document: this preserves the
  # Preferences inode/mtime and avoids serializing the rest of a potentially
  # multi-megabyte profile on every theme application. Any drift in the fields
  # we own still produces the same normalized replacement as before.
  ...
  if jq --argjson cs "$cs2" '
    def inir_theme_ready($wanted):
      (.extensions.theme.id == "") and
      (.extensions.theme.use_system == false) and
      (.extensions.theme.use_custom == false) and
      (.browser.theme.color_scheme == $wanted) and
      (.browser.theme.color_scheme2 == $wanted) and
      (((.browser.theme // {}) | has("user_color")) | not) and
      (((.browser.theme // {}) | has("user_color2")) | not);
    if inir_theme_ready($cs) then
      empty
    else
      .extensions.theme.id = "" |
  ...
  ' "$prefs_file" > "$tmp_file" 2>/dev/null; then
    if [[ -s "$tmp_file" ]]; then
      mv "$tmp_file" "$prefs_file"
      log "$name: preferences set (color_scheme=$cs2, color_scheme2=$cs2)"
    else
      rm -f "$tmp_file"
      log "$name: preferences already set (color_scheme=$cs2, color_scheme2=$cs2)"
    fi
  else
    rm -f "$tmp_file"
