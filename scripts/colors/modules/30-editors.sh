...
# Only deletes if the file was generated by iNiR (checks for known markers).
strip_neovim_spec() {
  if [[ -f "$NEOVIM_THEME_FILE" ]] && {
    grep -Fq '"yukazakiri/inir.nvim"' "$NEOVIM_THEME_FILE" ||
    grep -Fq '"bjarneo/aether.nvim"' "$NEOVIM_THEME_FILE" ||
    grep -Fq 'name = "inir-neovim"' "$NEOVIM_THEME_FILE"
  }; then
    rm -f "$NEOVIM_THEME_FILE"
  fi

  if [[ -f "$NEOVIM_OMARCHY_OVERRIDE_FILE" ]] && grep -Fq 'auto-generated by iNiR' "$NEOVIM_OMARCHY_OVERRIDE_FILE"; then
