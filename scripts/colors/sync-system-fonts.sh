...
def replace_line(path: str, pattern: str, replacement: str, prefix: str) -> None:
    ...
    # FontSyncService reconciles desktop settings on every shell start. Avoid
    # replacing GTK/xsettings files when they already contain the requested
    # value, preserving reconciliation without generating redundant disk I/O.
    if content == original:
        return
