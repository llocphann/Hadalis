...
declare -gA IPC_TARGET_DESC=(
  ...
  [codeWorkflowCapture]="Internal deterministic capture harness for Code Workflow UI regression tests. It is enabled only when the dedicated capture environment is active."
...
declare -gA IPC_TARGET_FUNCTIONS=(
  ...
  [codeWorkflowCapture]="begin status scenario restore"
...
declare -gA IPC_FUNCTION_DESC=(
  ...
  ["autostart:status"]="Return \`niri|<path>|<managedCount>|<externalCount>|<state>\`"
  ...
  ["background:clockDebugSetMode"]="Temporarily select a diagnostic clock mode"
  ...
  ["codeWorkflowCapture:begin"]="Snapshot the current Code Workflow UI/session state for a deterministic capture run"
  ["codeWorkflowCapture:status"]="Return the current Code Workflow capture state and geometry diagnostics as JSON"
  ["codeWorkflowCapture:scenario"]="Apply a named deterministic Code Workflow capture scenario"
  ["codeWorkflowCapture:restore"]="Restore the Code Workflow UI/session state saved by \`begin\`"
  ...
  ["settingsNav:page"]="Open the overlay and jump to page \`index\`"
...
declare -gA IPC_FUNCTION_ARGS=(
  ...
  ["codeWorkflowCapture:scenario"]="<name>"
...
IPC_ALL_TARGETS=(ai altSwitcher appCatalog audio autostart background bar brightness cheatsheet clipboard cliphistService closeConfirm codeWorkflowCapture codeWorkflowRuntime controlPanel coverflowSelector customWidgets dashboard dev gamemode globalActions keyboard lock mediaControls memory minimize mpris notifications osd osdInput osdVolume osk overlay overview packageSearch panelFamily recordingOsd region runtimeDiagnostics search session settings settingsNav shellLayout shellUpdate sidebarLeft sidebarRight taskview tiling voiceSearch wactionCenter waffleAltSwitcher wallpaperLauncher wallpaperSelector wbar widgetpower wnotificationCenter wwidgets ytmusic zoom)
...
declare -gA IPC_KEBAB_ALIASES=(
  ...
  [code-workflow-capture]=codeWorkflowCapture
