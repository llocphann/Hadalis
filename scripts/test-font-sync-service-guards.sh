...
require_literal "$helper" 'original = content' 'helper must retain the pre-edit bytes for no-op detection'
require_literal "$helper" 'if content == original:' 'helper must skip identical GTK/xsettings rewrites'
