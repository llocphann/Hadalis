...
reject "$cava" 'dataWatchdog.restart()' 'Cava hot frames must not restart the no-data watchdog Timer'
reject "$cava" 'dataWatchdog.stop()' 'Cava watchdog lifecycle must be binding-driven rather than manually toggled'
require "$cava" 'property var _watchdogState: ({ lastFrameMs: 0 })' 'Cava watchdog must keep frame heartbeat in non-notifying JS state'
require "$cava" 'running: root.active && cavaProc.running' 'Cava watchdog must only tick while the shared analyzer is active'
require "$cava" 'interval: 500' 'Cava watchdog must sample stalls at low cadence'
require "$cava" 'Date.now() - lastFrameMs >= 2400' 'Cava watchdog must preserve the 2.4 second stall threshold'
require "$cava" 'if (signalRelease.running)' 'Cava hot frames must not stop an already stopped release timer'
require "$cava" 'if (!root.audioSignalActive)' 'Cava hot frames must not rewrite an already active signal property'
