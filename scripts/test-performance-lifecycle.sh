...
require "$bar_cava_visualizer" 'property int waveStripCap: 96' 'Bar wave scene resolution must stay bounded'
require "$bar_cava_visualizer" 'Math.min(sourceCount, root.waveStripCap,' 'Bar wave level fan-out must honor the scene cap'
require "$repo_root/modules/bar/BarContent.qml" 'readonly property int barSpectrumWaveSampleCap: 96' 'Bar wave CAVA request must share the scene cap'
