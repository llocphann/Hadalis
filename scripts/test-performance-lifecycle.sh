...
sidebar_host="$repo_root/modules/sidebar/SidebarHost.qml"
...
require "$sidebar_host" 'readonly property int _presentationPollMaxTicks: 300' 'Sidebar presentation polling must have a bounded high-frequency burst'
require "$sidebar_host" 'function rearmPresentationPolling(): void' 'Sidebar presentation polling must be re-armable from loader/geometry events'
require "$sidebar_host" 'root._presentationPollTicks++' 'Sidebar unavailable presentation frames must count toward the polling bound'
require "$sidebar_host" 'root._presentationPollTicks >= root._presentationPollMaxTicks' 'Sidebar presentation polling must stop after the bounded burst'
require "$sidebar_host" 'presentationTimer.stop()' 'Sidebar presentation polling must be able to sleep on failure/closure'
require "$sidebar_host" 'root.rearmPresentationPolling()' 'Sidebar loader/geometry changes must wake a pending presentation request'
...
require "$local_music" 'property int _fallbackPollingConsumers: 0' 'Local Music must track presented fallback-poll consumers'
require "$local_music" 'root._fallbackPollingConsumers > 0' 'Local Music 900ms status polling must sleep without a presented Music view'
require "$local_music" '&& !root._mpdSubscriptionActive' 'Local Music 900ms status polling must remain subscription fallback-only'
require "$local_music" 'Qt.callLater(root.refreshStatus)' 'Local Music must refresh immediately when fallback polling gains its first consumer'
require "$local_music_view" 'root.visible && GlobalStates.sidebarLeftOpen' 'Local Music fallback polling demand must follow actual Sidebar presentation'
require "$local_music_view" 'LocalMusic.acquireFallbackPolling()' 'Presented Local Music view must acquire fallback polling demand'
require "$local_music_view" 'LocalMusic.releaseFallbackPolling()' 'Hidden/destroyed Local Music view must release fallback polling demand'
...
require "$resource_usage" 'command: ["/usr/bin/lscpu"]' 'CPU max-frequency detection must invoke lscpu directly'
