...
require "$bar_cava_visualizer" 'property bool _frameRebuildQueued: false' 'Bar CAVA geometry updates must be coalesced'
require "$bar_cava_visualizer" 'Qt.callLater(() => {' 'Bar CAVA points/ceiling notifications must collapse into one geometry rebuild'
...
reject "$bar_cava_visualizer" 'readonly property var _processedPoints:' 'Bar CAVA must not restore a multi-stage reactive frame geometry chain'
