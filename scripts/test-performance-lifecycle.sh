...
require "$dashboard" 'layer.enabled: root._slideLayerActive || contentLoader.desaturationActive' 'Dashboard slide must cache the heavy widget tree as one layer while moving'
require "$dashboard" 'layer.smooth: true' 'Dashboard motion cache must use smooth filtering'
