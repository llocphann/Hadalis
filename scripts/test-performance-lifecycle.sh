...
reject "$bar_cava_visualizer" 'property var _levels:' 'Bar CAVA must not broadcast one frame array through the whole QML binding graph'
reject "$bar_cava_visualizer" 'root._levels[index]' 'Bar delegates must not bind directly to a per-frame root array'
require "$bar_cava_visualizer" 'barRepeater.itemAt(i)' 'Bar CAVA frames must update stable delegates directly'
require "$bar_cava_visualizer" 'property real frameLevel: 0' 'Bar scene primitives must expose one scalar hot-frame input'
require "$bar_cava_visualizer" 'property var _waveFrameScratch: []' 'Bar wave resampling must reuse frame scratch'
require "$bar_cava_visualizer" 'onPointsChanged: root._applyFrame()' 'Bar CAVA must apply exactly one frame from the published points event'
