...
# Any pragma Singleton that lives directly in a declared local module must be
# exported by that module's qmldir. Missing this registration makes consumers
# fail at QML load time even though the implementation file itself is valid.
for qmldir in sorted(root.rglob('qmldir')):
    if any(part in skip_parts for part in qmldir.parts):
        continue
    module_dir = qmldir.parent
    try:
        qmldir_text = qmldir.read_text(encoding='utf-8')
    except UnicodeDecodeError as exc:
        errors.append(
            f'{qmldir.relative_to(root)}: unreadable qmldir: {exc}'
        )
        continue
    if not re.search(r'^\s*module\s+\S+', qmldir_text, re.MULTILINE):
        continue
    for impl in sorted(module_dir.glob('*.qml')):
        try:
            impl_text = impl.read_text(encoding='utf-8')
        except UnicodeDecodeError:
            continue
        if not re.search(r'^\s*pragma\s+Singleton\b', impl_text, re.MULTILINE):
            continue
        type_name = impl.stem
        registration = re.compile(
            rf'^\s*singleton\s+{re.escape(type_name)}\s+'
            rf'(?:\d+(?:\.\d+)?\s+)?{re.escape(impl.name)}\s*# properties/functions and other symbol references cannot survive while an old
...
,
        ...
        if not registration.search(qmldir_text):
            errors.append(
                f'{impl.relative_to(root)}: pragma Singleton is not exported '
                f'by {qmldir.relative_to(root)}'
            )
