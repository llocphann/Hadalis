...
assert.equal(created, 2, 'refreshed cached preview replaces its decoder exactly once');
assert.ok(!root.overviewWarmImages[12], 'missing preview never warms');
root.warmForOverview([11]); sync();
assert.equal(created, 2, 'reopening retains previously decoded Image object');
root.previewCache[12] = {path: '/tmp/previews/window-12.png', timestamp: 1};
root.warmForOverview([11,12]); sync();
assert.equal(created, 3, 'second existing preview warms into resident budget');
