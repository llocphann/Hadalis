...
class TranslationManager:
    ...
    def interactive_update(self, lang_code: str, missing_keys: Set[str], extra_keys: Set[str]):
        """Add missing keys and report source-orphan candidates without pruning them."""
        ...
        # Pruning must be source-driven across every locale. The dedicated
        # cleaner enforces key parity and applies one canonical orphan set, so
        # this per-locale updater only reports extra keys instead of deleting
        # them independently.
        if extra_keys:
            filtered_extra_keys = [
                key for key in extra_keys
                if not (
                    isinstance(translations.get(key, ""), str)
                    and translations.get(key, "").strip().endswith('/*keep*/')
                )
            ]
            if filtered_extra_keys:
                print(f"\nFound {len(filtered_extra_keys)} unused translation key candidates:")
                for i, key in enumerate(sorted(filtered_extra_keys), 1):
                    print(f"{i}. \"{key}\" -> \"{translations.get(key, '')}\"")
                print(
                    "Not deleting extra keys in per-locale update mode; "
                    "run translation-cleaner.py --clean so pruning is source-driven "
                    "and applied consistently to every locale."
                )
